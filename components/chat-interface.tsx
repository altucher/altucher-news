...
import { Send, User, Bot, Loader2, Plus, Newspaper, ExternalLink, Pencil, Lightbulb, Code, Search, Sparkles, Menu, X, MessageSquare, Trash2, LogOut, Zap, ImageIcon, Square, Globe, Paperclip, FileText, Brain, Mic, Volume2, VolumeX, Pickaxe, Music } from 'lucide-react'
...
const MINING_PROMPTS: Record<string, string> = {
  ...
  '22': "I want to start mining Bittensor Subnet 22 (Desearch). Act as my hands-on mining guide. Walk me through it step by step: (1) what Desearch rewards miners for (decentralized AI search — scraping and structuring real-time web and social/X data), (2) the mid-range server and bandwidth I need (no high-end GPU required), (3) installing bittensor, creating a wallet/hotkey, and registering on netuid 22 including TAO cost, (4) running the miner and returning fast, accurate, well-structured results, and (5) how to confirm my miner is scoring well. Start with step 1 and ask about my experience level and current setup.",
  '75': "I want to start mining Bittensor Subnet 75 (Hippius). Act as my hands-on mining guide. Walk me through it step by step: (1) what Hippius rewards miners for (decentralized cloud storage — IPFS-based and S3-compatible object storage, with no GPU required), (2) the storage server, disk capacity, and bandwidth I need, (3) installing bittensor, creating a wallet/hotkey, and registering on netuid 75 including TAO cost, (4) running the storage miner and maintaining reliable data availability, and (5) how to confirm my miner is scoring well. Start with step 1 and ask about my available storage and bandwidth.",
  ...
  '9': "I want to start mining Bittensor Subnet 9 (IOTA). Act as my hands-on mining guide, and be upfront that this is an advanced subnet focused on decentralized LLM pre-training. Walk me through it step by step: (1) what IOTA rewards miners for (contributing useful compute to collaboratively pre-train large language models), (2) the high-end GPU hardware and fast interconnect I realistically need, (3) installing bittensor, creating a wallet/hotkey, and registering on netuid 9 including TAO cost, (4) running the miner and contributing to training runs, and (5) how to confirm my miner is scoring well. Start with step 1 and ask about my GPU capacity and experience level.",
...
export default function ChatInterface() {
  ...
  useEffect(() => {
    scrollToBottom()
  }, [messages, newsHeadlines, generatedImages, generatingImage, generatedMusic, generatingMusic])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If the user is asking for music, route to music generation even
    // if they pressed the text/send button instead of the music button.
  ...
  const isMusicRequest = (text: string) => {
    ...
    return /\b(compose|hum)\b/.test(lowerText) ||
           lowerText.includes('generate music') ||
           lowerText.includes('generate a song') ||
           lowerText.includes('generate song') ||
           lowerText.includes('create music') ||
           lowerText.includes('create a song') ||
           lowerText.includes('make music') ||
           lowerText.includes('make a song') ||
           lowerText.includes('make me a song') ||
           lowerText.includes('write a song') ||
           lowerText.includes('write me a song') ||
           lowerText.includes('a song about') ||
           lowerText.includes('song about') ||
           lowerText.includes('compose a song') ||
           lowerText.includes('compose music') ||
           lowerText.includes('create a track') ||
           lowerText.includes('make a beat') ||
           lowerText.includes('generate a beat')
  }
                            ...
                            <div className="flex-1">
                              ...
                              <div className="relative rounded-lg border border-border bg-card p-4 max-w-md">
                                ...
                                <div className="mt-3 flex items-center justify-between">
                                  <a
                                    href={track.audioUrl}
                                    download={`bluetao-music-${track.id}.mp3`}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 text-emerald-600 bg-emerald-50 rounded-lg px-4 py-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-emerald-400">This may take 20-40 seconds</span>
                          </div>
                        </div>
                      </div>
                    </div>
