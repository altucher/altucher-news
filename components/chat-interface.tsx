...
const MINING_PROMPTS: Record<string, string> = {
  '1': "I want to mine Apex, Bittensor Subnet 1. Act as my hands-on Apex solver guide using the current official Macrocosmos workflow. First ask me: (1) whether I use Linux, macOS, or Windows with WSL2, (2) whether Python 3.12+ and Git are installed, (3) whether I already have a Bittensor coldkey/hotkey and TAO, and (4) my Python/ML experience. Then guide me one safe step at a time through cloning macrocosm-os/apex, running install_cli.sh, creating or selecting a wallet without ever asking me to share a mnemonic, registering the hotkey on netuid 1, running apex link, reviewing apex competitions and a competition baseline, submitting a conforming solution, and checking results. Be explicit that Apex is a winner-takes-all solver competition—not an always-on GPU inference miner—and warn me before any command that spends TAO.",
  ...
  useEffect(() => {
    if (status === 'submitted' && !phaseSetupDoneRef.current) {
      ...
      if (isNews) {
        ...
        phases.push(
          { status: codeMode ? 'Processing with Kimi K3...' : 'Routing through James...', details: [codeMode ? 'Running inference on Engy' : 'James picks the best model for this request', 'Pulling the relevant knowledge'] },
    ...
    <div className="flex h-screen bg-background">
      ...
      <aside className={cn(
        ...
        <nav className="px-3 pb-3 space-y-0.5">
          ...
          <Link href="/code" onClick={() => setSidebarOpen(false)}>
            <span className="w-full flex items-center gap-3 px-3 py-3 rounded-lg text-[0.9rem] font-medium text-[oklch(0.88_0.018_258)] hover:bg-sidebar-accent hover:text-[var(--gold)] transition-colors">
              <Bot className="w-5 h-5 flex-shrink-0" />
              code agent
            </span>
