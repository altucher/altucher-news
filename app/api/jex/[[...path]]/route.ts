/**
 * jex, the classifier API, after classifier.dev:
 *
 *   GET  /api/jex                          the docs, plain text
 *   GET  /api/jex/{labels}/{text}          one classification, bare label
 *   GET  /api/jex?labels=a,b&text=...      the same, query form
 *   POST /api/jex                          JSON in, JSON out, batches
 *
...
async function handle(req: NextRequest, ctx: { params: Promise<{ path?: string[] }> }) {
  ...
  // The raw, still percent-encoded path after /api/jex, so a %2C inside
  // a label survives. Next has already decoded ctx.params, which is why the
  // pathname is read instead.
  const { path: segments } = await ctx.params
  const rawPath = segments?.length ? url.pathname.replace(/^\/api\/jex\/?/, '') : ''
  ...
  if (req.method === 'POST') {
    ...
    if (getReq.nothing) {
      // Bare GET /api/jex: the documentation, exactly as the page shows it.
