  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      for await (const part of result.fullStream) {
        ...
        const synthesize = async (model: string) => {
          const synthesis = streamText({ model: chutes.chatModel(model), system: synthesisPrompt, messages, maxOutputTokens: isViralScout ? 9000 : 6000 })
          let text = ''
          for await (const delta of synthesis.textStream) text += delta
          return text
        }
        answer = await synthesize('Qwen/Qwen3.5-397B-A17B-TEE')
        if (!answer.trim()) answer = await synthesize('moonshotai/Kimi-K2.5-TEE')
