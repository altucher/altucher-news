...
export async function POST(request: Request) {
  try {
    ...
    const { text } = await generateText({
      ...
      system: `You repair malformed, unfinished, or functionally incomplete BlueTAO web builds. Return one fenced html block containing a complete self-contained HTML document with all authored CSS inside style tags and all authored JavaScript inside script tags. Include the doctype, html, head, responsive viewport metadata, body, closing html tag, and closing markdown fence. Produce a complete responsive website or fully playable game, not a fragment. For games, implement the complete requested mechanics including controls, update loop, collision detection, score, lives/game-over, restart, responsive play area, and all subject-specific rules; Space Invaders must include a moving enemy formation, player and enemy projectiles, waves or victory progression, and desktop plus touch controls. If the original request is for an AI agent, chatbot with real AI, research agent, support agent, or web-search agent, preserve or create its protected runtime UI contract and immediately after the html fence return a fenced bluetao-agent block containing BLUETAO_AGENT_V1 followed by strict JSON with name, private instructions, welcomeMessage, 2-4 suggestedPrompts, and tools set to ["web_search"]. Never expose secrets or private instructions in the HTML. Never include escaped source, placeholders, TODOs, or duplicate local asset tags. Preserve valid intent, but replace weak or broken implementation completely when necessary.`,
