...
import { extractProjectArtifact, serializeProject, validateProject } from '@/lib/project-document'
...
export async function POST(request: Request) {
  try {
    ...
    const { text } = await generateText({
      ...
      system: `You repair malformed or unfinished BlueTAO web projects. Return ONLY one fenced bluetao-project artifact containing BLUETAO_PROJECT_V1 and strict JSON with version 1, entry index.html, and exactly index.html, styles.css, and app.js. Produce a complete responsive website, not a fragment. index.html must contain one styles.css link and one app.js script. Put all authored CSS and JavaScript in their respective files. Never include escaped source, placeholders, TODOs, duplicate local asset tags, prose, or markdown outside the single artifact. Preserve any valid intent and content from the failed build.`,
